...
const AttendanceTable = ({ data }) => {
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      setTableData(
        data.map((row) => {
          const newRow = { ...row };
          newRow["date"] = ""; // Add an empty date field for each row
          newRow["time"] = ""; // Add an empty time field for each row
          delete newRow[Object.keys(row)[0]]; // remove unnecessary column
          return newRow;
        })
      );
  ...
  // Function to handle individual date or time changes
  const handleInputChange = (rowIndex, col, value, type) => {
    setTableData((prevData) => {
      return prevData.map((row, index) => {
        if (index === rowIndex) {
          // Update the correct column (either "date" or "time")
          return {
            ...row,
            [col]: value, // Update the value of the specific column
          };
        }
        return row;
      });
                ...
                >
                  {rowIndex === 1 && colIndex >= 2 ? (
                    <input
                      ...
                      value={row[col] || ""}
                      onChange={(e) =>
                        handleInputChange(rowIndex, col, e.target.value, "date")
                      }
                      className="p-2 border border-gray-300 rounded mr-4"
                    />
                  ) : rowIndex === 2 && colIndex >= 2 ? (
                    <input
                      ...
                      value={row[col] || ""}
                      onChange={(e) =>
                        handleInputChange(rowIndex, col, e.target.value, "time")
                      }
                      className="p-2 border border-gray-300 rounded mr-4"
                    />
                  ) : rowIndex >= 3 && colIndex >= 2 ? (
